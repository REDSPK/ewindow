...
class Ion_auth
{
	...
	public function register($username, $password, $email, $additional_data = array(), $group_name = array()) //need to test email activation
	{
		...
		else
		{
			...
			if (!$id) //if intertion in database fails. return false
			{
				$this->set_error('account_creation_unsuccessful');
				return FALSE;
			}
			...
			if (!$deactivate) //if deactivation fails for user. when the user aaccount is created with e-mail activation ON. The account is deactivated unless activated by mail
			{
