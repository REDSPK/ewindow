...
//                $default_group =  $this->db->get_where('groups',array('name'=>$this->config->item('default_group', 'ion_auth')))->row_array();
                
                /*Create default wishlist for user */
                $this->create_wishlist($id);
	...
	public function user($id = NULL)
	{
        ...
        public function is_my_shop($shopID) {
            $user = $this->user()->row();
            $query = $this->db->get_where('shop',array('owner' =>$user->user_id,'shop_id' => $shopID));
            if( $query->num_rows()>0) {
                return true;
            }
            else {
	...
	**/
	...
	protected function _filter_data($table, $data)
	{
        ...
        function create_wishlist($userID) {
            $data = array(
                        'user_id' => $userID ,
